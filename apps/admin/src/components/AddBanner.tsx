...
import { Loader2, Image as ImageIcon, Plus, X, Link as LinkIcon } from "lucide-react";
...
const BannerFormSchema = z.object({
    title: z.string().trim().min(1, "Vui lòng nhập tiêu đề banner").max(200, "Tiêu đề tối đa 200 ký tự"),
    description: z.string().trim().max(2000, "Mô tả tối đa 2000 ký tự").optional(),
    imageUrl: z.string().trim().min(1, "Vui lòng tải lên hình ảnh banner").max(500, "Đường dẫn hình ảnh tối đa 500 ký tự"),
    targetType: z.enum(["PRODUCT", "CATEGORY", "EXTERNAL_LINK"]),
    targetId: z.coerce.number().optional(),
    linkUrl: z.string().trim().max(500, "Đường dẫn liên kết tối đa 500 ký tự").optional(),
    displayOrder: z.coerce.number().default(0),
    isActive: z.boolean().default(true),
}).refine((data) => {
    // Ràng buộc: Nếu là Link ngoài thì bắt buộc phải nhập linkUrl
    if (data.targetType === "EXTERNAL_LINK" && !data.linkUrl) return false;
    return true;
}, {
    message: "Vui lòng nhập đường dẫn liên kết",
    path: ["linkUrl"],
}).refine((data) => {
    // Ràng buộc: Nếu là SP hoặc Danh mục thì bắt buộc phải chọn ID
    if ((data.targetType === "PRODUCT" || data.targetType === "CATEGORY") && !data.targetId) return false;
    return true;
}, {
    message: "Vui lòng chọn đối tượng đích",
    path: ["targetId"],
});
...
const AddBanner = () => {
    ...
    // Lấy giá trị targetType hiện tại để render động UI
        ...
        <SheetContent className="sm:max-w-[600px] p-0">
            <ScrollArea className="h-full p-6">
                <SheetHeader className="mb-6">
                    ...
                    <SheetDescription>
                        Thiết kế banner hiển thị trên trang chủ để thu hút khách hàng.
                    </SheetDescription>
                </SheetHeader>
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit((data) => mutation.mutate(data))} className="space-y-6 pb-20">
                        ...
                        <div className="p-4 border rounded-lg bg-gray-50/50 shadow-sm">
                            <h3 className="font-semibold text-sm uppercase text-gray-500 mb-4">Hình ảnh Banner (Bắt buộc)</h3>
                            <div className="flex flex-col items-center justify-center">
                                {form.watch("imageUrl") ? (
                                    ...
                                    <label className="w-full h-40 border-2 border-dashed rounded-md flex flex-col items-center justify-center cursor-pointer hover:bg-blue-50 transition border-blue-200 bg-white">
                                        ...
                                        <span className="text-xs text-gray-400 mt-1">Hỗ trợ JPG, PNG (Khuyến nghị 1920x600)</span>
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl><Input placeholder="VD: Siêu sale 11/11..." {...field} /></FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            />
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl><Textarea className="h-20" placeholder="Chi tiết chương trình..." {...field} /></FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            />
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                                <FormControl><SelectTrigger className="bg-white"><SelectValue placeholder="Chọn loại" /></SelectTrigger></FormControl>
                                                <SelectContent>
                        ...
                        <div className="grid grid-cols-2 gap-4 p-4 border rounded-lg shadow-sm">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>Thứ tự hiển thị (0 là đầu tiên)</FormLabel>
                                        <FormControl>
                                            <Input
                                                type="number"
                        ...
                        <Button type="submit" className="w-full h-12 text-lg font-bold shadow-lg" disabled={mutation.isPending}>
                            {mutation.isPending ? <Loader2 className="animate-spin mr-2" /> : "Tạo banner"}
                        </Button>
                    </form>
                </Form>
            </ScrollArea>
